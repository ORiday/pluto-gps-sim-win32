...
#include <ad9361.h>
#include "getopt.h"
#include "pluto-gps-sim.h"

#define RINEX_FILE_NAME "brdc%j0.%yn.Z"
#define RINEX_FTP_URL "cddis.gsfc.nasa.gov"
#define RINEX_FTP_FOLDER "/gnss/data/daily/%Y/brdc/"
...
struct stream_cfg {
	...
	HANDLE data_cond;
	HANDLE tx_thread;
	HANDLE data_mutex; // Mutex to synchronize buffer access
	const char *uri;
	const char *hostname;
...
static HANDLE pluto_thread;
static DWORD pluto_thread_id;
...
static int readRinexNavAll(ephem_t eph[][MAX_SAT], ionoutc_t *ionoutc, const char *fname) {
	...
	while (1) {
		...
		if (strncmp(str + 60, "END OF HEADER", 13) == 0)
			...
			strncpy_s(tmp, str + 3, 19);
			tmp[19] = 0;
			replaceExpDesignator(tmp, 19);
			ionoutc->A0 = atof(tmp);

			strncpy_s(tmp, str + 22, 19);
			tmp[19] = 0;
			replaceExpDesignator(tmp, 19);
			ionoutc->A1 = atof(tmp);

			strncpy_s(tmp, str + 41, 9);
			tmp[9] = 0;
			ionoutc->tot = atoi(tmp);

			strncpy_s(tmp, str + 50, 9);
			tmp[9] = 0;
			...
			strncpy_s(rinex_date, str + 40, 15);
			rinex_date[15] = 0;
		}
	}
	...
	while (1) {
		...
		// EPOCH
		strncpy_s(tmp, str + 3, 2);
		tmp[2] = 0;
		t.y = atoi(tmp) + 2000;

		strncpy_s(tmp, str + 6, 2);
		tmp[2] = 0;
		t.m = atoi(tmp);

		strncpy_s(tmp, str + 9, 2);
		tmp[2] = 0;
		t.d = atoi(tmp);

		strncpy_s(tmp, str + 12, 2);
		tmp[2] = 0;
		t.hh = atoi(tmp);

		strncpy_s(tmp, str + 15, 2);
		tmp[2] = 0;
		t.mm = atoi(tmp);

		strncpy_s(tmp, str + 18, 4);
		tmp[2] = 0;
		t.sec = atof(tmp);
		...
		strncpy_s(tmp, str + 41, 19);
		tmp[19] = 0;
		replaceExpDesignator(tmp, 19);
		eph[ieph][sv].af1 = atof(tmp);

		strncpy_s(tmp, str + 60, 19);
		tmp[19] = 0;
		replaceExpDesignator(tmp, 19);
...
DWORD WINAPI pluto_tx_thread_ep(LPVOID lpParameter) {
	...
	ad9361_set_bb_rate(iio_context_find_device(ctx, "ad9361-phy"), (unsigned long)plutotx.fs_hz);
...
int main(int argc, char *argv[]) {
	...
	plutotx.data_mutex = CreateMutex(NULL, FALSE, NULL);
	plutotx.data_cond = CreateEvent(NULL, FALSE, FALSE, NULL);
	...
	*/
	...
	while ((result = getopt(argc, argv, "e:m:u:g:c:l:s:T:t:A:B:U:N:vfiL?")) != -1) {
		...
		case 't':
			sscanf_s(optarg, "%d/%d/%d,%d:%d:%lf", &t0.y, &t0.m, &t0.d, &t0.hh, &t0.mm, &t0.sec);
	...
	*/
	...
	////////////////////////////////////////////////////////////
	if (use_ftp) {
    	fprintf(stderr, "Downloading ephemeris data from server...\n");
		time_t t = time(NULL);
		struct tm tm;
		localtime_s(&tm, &t);
		char* folder = (char*)malloc(MAX_PATH);
		strftime(folder, MAX_PATH, RINEX_FTP_FOLDER, &tm);
		char* filename = (char*)malloc(MAX_PATH);
		strftime(filename, MAX_PATH, RINEX_FILE_NAME, &tm);

		char buf[1024];
		DWORD err, len;

		HINTERNET hInternetRoot = InternetOpen(TEXT("ftp-client"), INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, NULL);
		...
		if(!FtpSetCurrentDirectoryA(hFtpSession, folder)) {
			err = GetLastError();
			len = sizeof(buf);
	...
	// Allocate I/Q buffer
	iq_buff = (short*)calloc(NUM_SAMPLES, 4);
	...
	////////////////////////////////////////////////////////////
	pluto_thread = CreateThread(0, 0, pluto_tx_thread_ep, NULL,  0, &pluto_thread_id);
	...
	while (!plutotx.exit) {
...
#endif
					ip = chan[i].dataBit * chan[i].codeCA * cosTable512[iTable] * (int)gain[i];
					qp = chan[i].dataBit * chan[i].codeCA * sinTable512[iTable] * (int)gain[i];
		...
		ReleaseMutex(plutotx.data_mutex);
		WaitForSingleObject(plutotx.data_cond, INFINITE);
...
exit_main_thread:
	ReleaseMutex(plutotx.data_mutex);
	plutotx.exit = true;
	WaitForSingleObject(pluto_thread, INFINITE);
	CloseHandle(pluto_thread); /* Wait on Pluto TX thread exit */
	CloseHandle(plutotx.data_mutex);
	CloseHandle(plutotx.data_cond);
